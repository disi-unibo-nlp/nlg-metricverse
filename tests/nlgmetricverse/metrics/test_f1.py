...
def test_basic(
    predictions, references, nlgmetricverse_f1, output_basic
):
    scores = nlgmetricverse_f1(predictions=predictions, references=references)
    assert_almost_equal_dict(actual=scores, desired=output_basic)


def test_multiple_ref(
    predictions, multiple_references, nlgmetricverse_f1, output_multiple_ref
):
    scores = nlgmetricverse_f1(predictions=predictions, references=multiple_references)
...
def test_multiple_pred_multiple_ref(
    ...
    scores = nlgmetricverse_f1(predictions=multiple_predictions, references=multiple_references)
