...
def test_basic(predictions, references, nlgmetricverse_rouge, output_basic):
    scores = nlgmetricverse_rouge(predictions=predictions, references=references)
    assert_almost_equal_dict(actual=scores, desired=output_basic)


def test_multiple_ref(predictions, multiple_references, nlgmetricverse_rouge, output_multiple_ref):
    scores = nlgmetricverse_rouge(predictions=predictions, references=multiple_references)
    assert_almost_equal_dict(actual=scores, desired=output_multiple_ref)


def test_multiple_pred_multiple_ref(
    multiple_predictions, multiple_references, nlgmetricverse_rouge, output_multiple_pred_multiple_ref
):
    scores = nlgmetricverse_rouge(predictions=multiple_predictions, references=multiple_references)
