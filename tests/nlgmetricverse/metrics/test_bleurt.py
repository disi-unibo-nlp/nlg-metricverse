...
def test_basic(predictions, references, nlgmetricverse_bleurt, output_basic):
    scores = nlgmetricverse_bleurt(predictions=predictions, references=references)
    assert_almost_equal_dict(actual=scores, desired=output_basic)


def test_multiple_ref(predictions, multiple_references, nlgmetricverse_bleurt, output_multiple_ref):
    scores = nlgmetricverse_bleurt(predictions=predictions, references=multiple_references)
    assert_almost_equal_dict(actual=scores, desired=output_multiple_ref)


def test_multiple_pred_multiple_ref(
    multiple_predictions, multiple_references, nlgmetricverse_bleurt, output_multiple_pred_multiple_ref
):
    scores = nlgmetricverse_bleurt(predictions=multiple_predictions, references=multiple_references)
