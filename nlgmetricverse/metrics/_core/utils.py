    ...
    with open(path) as f:
        for line in f:
            if "upper_bound" in line:
                upper_bound_str = line.split("=")[1].strip()
                upper_bound = float(upper_bound_str.replace(",", ""))
            if "lower_bound" in line:
                lower_bound_str = line.split("=")[1].strip()
                lower_bound = float(lower_bound_str.replace(",", ""))
